...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "js-to-html", "like-ar", "typed-controls", "type-store", "dialog-promise", "best-globals", "myOwn"], factory);
    }
    ...
    var tipoc_Base = /** @class */ (function () {
        ...
        tipoc_Base.prototype.connectControl = function (control) {
            ...
            if (this.data.tipovar === 'hora' && !(actualValue instanceof bestGlobals.timeInterval)) {
                actualValue = actualValue ? TypeStore.typerFrom(exports.formTypes[this.data.tipovar]).fromString(actualValue) : null;
            }
    ...
    var tipoc_BF = /** @class */ (function (_super) {
        ...
        tipoc_BF.prototype.adaptOptionInput = function (groupElement) {
            ...
            var completarTablaResumen = function completarTablaResumen(table, formData, navigationButton, maxFieldsCount, mostrarUnidadesAnalisisEnResumen, searchFormId) {
                var thArray = [];
            ...
            var createRowView = function createRowView(row, iPosition) {
                ...
                if (cantResumen) {
                    ...
                    var tr = js_to_html_1.html.tr({ class: 'row' }, [
                        js_to_html_1.html.td({ id: 'despliegue-formulario-' + nombreFormulario + '-' + (iPosition + 1).toString(), colspan: (cantResumen + 1) }).create()
                    ]).create();
                    table.appendChild(tr);
                }
                else {
                    var span = js_to_html_1.html.span({ id: 'despliegue-row-hijo-' + nombreFormulario + '-' + (iPosition + 1).toString() }, [
                        button,
                        js_to_html_1.html.span({ id: 'despliegue-formulario-' + nombreFormulario + '-' + (iPosition + 1).toString() }, []).create(),
                    ]).create();
                    ...
                    if (estructura.variables[miVariable].calculada) {
                        ...
                        if (estructura.variables[miVariable].tipo == 'opciones') {
                            ...
                            if (!(valor instanceof bestGlobals.timeInterval)) {
                                valor = valor ? TypeStore.typerFrom(exports.formTypes['hora']).fromString(valor) : null;
                            }
