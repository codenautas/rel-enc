...
const dialog_promise_1 = require("dialog-promise");
const my_things_1 = require("my-things");
var formTypes = {
...
class tipoc_Base {
    ...
    displayRef(opts = {}) {
        ...
        return [js_to_html_1.html[opts.forValue ? 'label' : (this.inTable ? 'td' : 'span')](attr, hasValue ? this.data.ver_id || this.data.casillero : null)];
    }
    displayInput(direct = false) {
        ...
        return js_to_html_1.html[this.inTable ? 'td' : 'span'](attr, [control]).create();
    }
    displayMainText(opts = {}) {
        ...
        return [
            js_to_html_1.html[opts.forValue ? 'label' : (this.inTable ? 'td' : 'span')](attr, [
                this.data.nombre,
    ...
    displayChilds() {
        return js_to_html_1.html.div({ class: "hijos" }, [].concat(this.childs.map(function (child) {
            ...
            if (casillerosElement.length > 0) {
                casillerosElement[0].addEventListener('click', function () {
                    dialog_promise_1.miniMenuPromise([
                        { value: 'next', img: my_things_1.my.path.img + 'next.png', label: 'próxima pregunta', doneFun: function () {
                                self.myForm.irAlSiguiente(self.var_name, true);
                            } },
                        { value: 'delete', img: my_things_1.my.path.img + 'delete.png', label: 'borrar respuesta', doneFun: function () {
    ...
    displayRef(opts = {}) {
        ...
        if (myForm.back.pilaDeRetroceso.length) {
            ...
            button.onclick = function () {
                ...
                mainForm.innerHTML = '';
                mainForm.appendChild(my_things_1.my.displayForm(myForm.surveyStructure, myForm.back.row, myForm.back.formId, myForm.back.pilaDeRetroceso.slice(1)));
                window.scrollTo(0, 0);
            };
        }
            ...
            opcion.childs.forEach(function (pregunta) {
                ...
                if (!foundedColumns[pregunta.data.nombre]) {
                    foundedColumns[pregunta.data.nombre] = {
                        ...
                        html: js_to_html_1.html.td({ class: 'pmatriz_titulo_columna', "casillero-id": pregunta.data.padre + '/' + pregunta.data.casillero }, pregunta.data.nombre)
                    };
...
class tipoc_BF extends tipoc_Base {
    adaptOptionInput(groupElement) {
        ...
        var loadForm = function loadForm(formName, rowHijo, UAdelForm, iPosicional) {
            var mainForm = document.getElementById('main-form');
            mainForm.innerHTML = '';
            mainForm.appendChild(my_things_1.my.displayForm(myForm.surveyStructure, rowHijo, formName, [
        ...
        if (myForm.depot.row[UAdelForm]) {
            if (PuedeAgregarRenglones) {
                ...
                button.onclick = function () {
                    my_things_1.my.ajax.cargar.preguntas_ua({ operativo: sessionStorage.getItem('operativo'), unidad_analisis: UAdelForm }).then(function (result) {
                        var object = {};
...
class FormStructure {
    ...
    posicionarVentanaVerticalmente(control, y) {
        var rect = my_things_1.my.getRect(control);
        if (rect.top) {
