...
var tipoc_F = /** @class */ (function (_super) {
    ...
    ;
    tipoc_F.prototype.createBackButton = function (position) {
        var myForm = this.myForm;
        if (myForm.stackLength()) {
            var firstFromStack = myForm.getFirstFromStack();
            var button = js_to_html_1.html.button({ id: 'volver-a-' + firstFromStack.formName + '-' + position + '-from-' + myForm.formId, "class": 'boton-formulario', "enter-clicks": true }, "Volver al " + firstFromStack.formName).create();
...
var tipoc_BF = /** @class */ (function (_super) {
    ...
    tipoc_BF.prototype.adaptOptionInput = function (groupElement) {
        ...
        var loadForm = function loadForm(nombreFormulario, formData, formAnalysisUnit, iPosition, myForm, callerButton) {
        ...
        var createFormButton = function createFormButton(formName, buttonDescription, myForm, rowHijo, formAnalysisUnit, iPosition) {
            var button = js_to_html_1.html.button({ id: 'ver-' + formName + '-' + iPosition, "class": 'boton-formulario' }, buttonDescription).create();
            button.onclick = function () {
                        ...
                        if (mostrarUnidadesAnalisisEnResumen) {
                            ...
                            if (formData[pregunta.var_name].length) {
                                formData[pregunta.var_name].forEach(function (childFormData, index) {
                                    ...
                                    if (formIdForUa) {
                                        var button = js_to_html_1.html.button({ id: 'ver-' + formIdForUa + '-' + (index + 1).toString(), "class": 'boton-formulario' }, formIdForUa + ' ' + (index + 1)).create();
                                        button.onclick = function () {
        ...
        if (myForm.formData[formAnalysisUnit]) {
            ...
            if (PuedeAgregarRenglones) {
                var newButton = js_to_html_1.html.button({ id: 'boton-nuevo-' + nombreFormulario, "class": 'boton-nuevo-formulario', "enter-clicks": true }, "Nuevo " + nombreFormulario).create();
                var self = this;
                ...
                var div = js_to_html_1.html.div({ id: 'boton-listo-' + nombreFormulario, "class": 'nuevo-formulario' }, [newButton, readybutton]).create();
...
var FormManager = /** @class */ (function () {
    ...
    FormManager.prototype.irAlSiguiente = function (variableActual, scrollScreen) {
        var control;
        if (exports.globalSaltosABotones[variableActual] && (exports.globalSaltosABotones[variableActual][this.formData[variableActual]] ||
            exports.globalSaltosABotones[variableActual].siempre)) {
            var id = exports.globalSaltosABotones[variableActual][this.formData[variableActual]] ||
                exports.globalSaltosABotones[variableActual].siempre;
    ...
    FormManager.prototype.irAlSiguienteDespliegue = function (primeraVariable, elementToFocusIfNotActual) {
        var actual = primeraVariable;
        if (actual) {
            var controles = this.controls;
            var formManager = this;
            var tieneSiguienteObligatoriaVacia = function tieneSiguienteObligatoriaVacia(variable, controles) {
                var seguir = true;
            ...
            var todasLasVariablesVacias = function todasLasVariablesVacias(variable, controles) {
                var seguir = true;
                while (formManager.state.siguientes[variable] && seguir) {
                    var variable = formManager.state.siguientes[variable];
                    var control = controles[variable];
